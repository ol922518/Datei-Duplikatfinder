...
dort muss `PySide6` installiert sein). Für ein eigenes Programm-Icon im
Dock/Finder lässt sich `AppIcon.icns` per Finder → „Informationen“ → Icon
oben links anklicken → einfügen auf die `.command`-Datei anwenden.
